...
name_list_s = sorted(glob.glob('C:\\Tomi\\AA_SZTE_Mester\\Studies\\Diplomamunka\\4_felev\\Advanced_2\\GOES_10_12\\*.LIS'))
name_list_g = sorted(glob.glob('C:\\Tomi\\AA_SZTE_Mester\\Studies\\Diplomamunka\\4_felev\\Advanced_2\\GOES_10_12\\*.CSV'))
    ...
    if (sc == xray):
        ...
        for i in range (0,len(xclass_dataline)):
            ...
            if(yt == sc):
                ...
                plt.xlabel('')
                plt.xticks([begin_new, pt1, end_new])
                plt.xlim(begin_new, end_new)
                plt.ylim([min(fwa), max(fwa)])
                plt.ylabel('Frequency(Hz)')
                plt.title("%s %s Schumann resonance (weighted average frequency)\n and X-ray radiation flux (1 - 8 nm)" % (source_file_s1[-14:-4], peak_string))
                ...
                plt.xticks([begin_new, pt1, end_new])
                plt.xlim(begin_new, end_new)
                plt.ylabel('X-ray radiation flux (W/m^2)')
                ...
                plt.ylabel('Frequency(Hz)')
                plt.title("%s %s; Schumann resonance 1 - 3. mode\n and X-ray radiation flux (1 - 8 nm)" % (source_file_s1[-14:-4], peak_string))
                plt.axvline(peak[i], 0, 4, color='r')
